...
#include "FollowEvaluator.h"


//Handle both wild : e.g Relation(_, _)
bool FollowEvaluator::haveRelation() {
	return pkb.getFollows().empty();
}

//Handle both constant : e.g Relation(1, 2)
bool FollowEvaluator::isRelation(Entity e1, Entity e2) {
...
ResultTable FollowEvaluator::getRightRelations(Entity header) {
	return ResultTable(header, pkb.getFollowing());
}

//Handle right declartion, left constant: e.g Relation(a, _)
ResultTable FollowEvaluator::getLeftRelations(Entity header) {
	return ResultTable(header, pkb.getFollowed());
}

//Handle left constant, right declartion: e.g Relation(1, a)
ResultTable FollowEvaluator::getRelationMatchLeft(Entity constant, Entity header) {
...
ResultTable FollowEvaluator::getRelationMatchRight(Entity header, Entity constant) {
	stmt_index c = stoi(constant.getValue());
